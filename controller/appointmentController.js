...
export const getAllAppointments = async (req, res) => {
  try {
    ...
    const appointments = await Appointment.find({ adminId: _id })
...
export const getAppointedTimeSlots = async (req, res) => {
  try {
    const {
      adminId = '',
      date = ''
    ...
    if (!adminId || !date) {
      return res.status(400).json({
        success: false,
        message: 'adminId and date are required query parameters'
      });
    }

    const appointments = await Appointment.find({ adminId, date }).select('time -_id');

    const timeSlots = appointments.map(a => a.time);
